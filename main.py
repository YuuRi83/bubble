...
def fetch_naver_news(category, is_extreme):
    ...
    params = {"query": search_query, "display": 20, "sort": "sim"}
...
def get_feed():
    ...
    for cat, count in cat_counts.items():
        ...
        if count <= len(titles_pool):
            sampled = random.sample(titles_pool, count)
        else:
            sampled = random.choices(titles_pool, k=count)

        for title in sampled:
            feed.append({"title": title, "category": cat, "is_extreme": is_extreme})
    random.shuffle(feed)
    return feed
